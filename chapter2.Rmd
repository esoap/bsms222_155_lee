...
![Figure 1.1](/Users/USER/Desktop/bsms222_155_lee/images/murder-rate-example-1-1.png)

Or even worse, this version from everytown.org:

![Figure 1.2](/Users/USER/Desktop/bsms222_155_lee/images/murder-rate-example-2-1.png)
...
![Figure 1.3](/Users/USER/Desktop/bsms222_155_lee/images/us-murders-by-state-map-1.png)
...
$$
\frac{-b +\sqrt{b^2 - 4ac}}{2a}\quad and \quad \frac{-b - \sqrt{b^2 - 4ac}}{2a}
$$
...
![Figure 2.1](/Users/USER/Desktop/bsms222_155_lee/images/rstudio-environment.png)
...
# what the writer intended was to print to the decimal place 3 and 2
# tried using the round() function to decimal place 3 and 2

round((-b + sqrt(b^2 - 4*a*c) ) / ( 2*a ), digits = 3)
round((-b - sqrt(b^2 - 4*a*c) ) / ( 2*a ), digits = 2)

# interestingly "number 5" "goes to the even digit
# example: round(0.5) would result in 0, not 1
```
...
*suddenly the `murder$p` turns into `murder$population` (or show possible parameters, variable etc) $NEAT$*
...
*(was unable to find the way to make tables in R markdown, replaced it with the image)*
![Figure 9.1](/Users/USER/Desktop/bsms222_155_lee/images/summarize.png)
...
```{r}
state_no <- which.min(murders$population)
...
7. The `na_exampl`e vector represents a series of counts. You can quickly examine the object using:
...
![Figure 11.1](/Users/USER/Desktop/bsms222_155_lee/images/vectors-add.png)
...
# used the round function to get a touch of real-data-like-feeling
temp_c <- round(5/9*(temp_f - 32), digits=1)
city <- c("Beijing", "Lagos", "Paris", "Rio de Janeiro", 
          "San Juan", "Toronto")
city_temps <- data.frame(name = city, Celcius_temperature = temp_c, Farenheit_temperature= temp_f)
city_temps
```
...
# checked if the result is similar to π^2/6
pi^2/6
```
...
```{r}
murder_per_100k <- murders$total / murders$population * 100000
mean(murder_per_100k)
```
...
```{r}
ne_low <- low & murders$region=="Northeast" 
```
...
```{r}
refer_abbs <- c("AK", "MI", "IA")
ind <- match(refer_abbs, murders$abb)
murders$state[ind]
```
